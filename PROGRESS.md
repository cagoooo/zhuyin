## 🎮 v3.6.1 已完成功能 (Phase 6: UI 修復)
- [x] **🔘 重置按鈕排版優化**：由於關卡數量大幅增加，原本使用 `absolute` 定位的「重置進度」按鈕會隨著畫面捲動而遮擋到最底部的關卡卡片。現已將其改為 `fixed` 定位，使其永遠固定在畫面右下角，不會影響任何介面點擊。

---

## 🎮 v3.6.0 已完成功能 (Phase 6: 題庫大規模擴充)
- [x] **📚 新增四大關卡類別**：全新加入了「自然 🌳」(山、海、花等)、「昆蟲 🦋」(蝶、蜂、蟻等)、「動作 🏃」(跑、跳、笑等) 與「日常 🏠」(床、杯、衣等) 共 4 個大關卡，為小朋友帶來更豐富的生活詞彙！
- [x] **🍎 原有关卡容量升級**：為「水果」分類加入了 (鳳梨的鳳、芭樂的芭、芒果的芒)；為「動物」分類加入了常見農場動物 (牛、羊、馬、豬、鴨)，大幅增加遊戲耐玩度與學習廣度。
- [x] **🎨 專屬主題色彩配對**：為新增加的四個關卡分別調配了專屬的 CSS 漸層背景色，確保遊玩體驗中的視覺多樣性。

---

## 🎮 v3.5.8 已完成功能 (Phase 5: 核心漏洞修復)
- [x] **🧩 字圖不符 Bug 修復**：移除了題庫中因外部圖庫 (icons8) 連結失效而無法載入的舊圖片。為了避免系統一律使用「關卡類別大圖」(例如：水果關卡統一顯示 🍎) 進行保底，導致「字是檸檬，圖是蘋果」的錯亂情況，目前已為這 15 個缺漏圖片的單字補上專屬的 Emoji 表情符號 (例如：檬顯示 🍋、弟顯示 👦)，精準對應題意！

---

## 🎮 v3.5.7 已完成功能 (Phase 5: 視覺優化)
- [x] **🔍 聲調按鈕 UI 優化**：大幅放大了鍵盤區下方的四個聲調符號 (ˊ、ˇ、ˋ、˙)，並微調了內部間距 (`padding-top: 10px;`)，抵銷了這幾個標點符號先天上偏向字元頂部的排版特性，現在它們看起來更巨大、更清楚且完美置中了！

---

## 🎮 v3.5.6 已完成功能 (Phase 5: 響應式優化)
- [x] **📜 遊戲畫面防裁切 (RWD)**：修復了在螢幕較狹長的設備上，遊戲畫面 (#game-screen) 底部的注音鍵盤與「送出作答 / 清除」按鈕可能被裁切且無法捲動的問題。透過為該區域補上原生 `overflow-y: auto`，確保所有核心控制元件皆可被順利觸及。

---

## 🎮 v3.5.5 已完成功能 (Phase 5: 響應式優化)
- [x] **📜 地圖選單滾動優化 (RWD)**：修復了手機與平板設備在「選擇關卡」畫面因高度不足而無法看見底部按鈕（如：三角形、漢堡關卡）的問題。為選單畫布套用了原生的滑動遮罩與彈性捲動 (`scrollable-screen`)，提升跨平台使用體驗。

---

## 🎮 v3.5.4 已完成功能 (Phase 5: 視覺優化)
- [x] **✨ 遊戲進度條 & 計時器美化**：重構了遊戲畫面頂部的 UI 佈局。進度條全面升級為 Claymorphism 風格，增加內部高光與 3D 陰影層次；計時器亦改為更明顯的膠囊狀紅框白底樣式，使視覺更一致且充滿活力。

---

## 🎮 v3.5.3 已完成功能 (Phase 5: 行動裝置支援與鍵盤優化)
- [x] **📱 iOS/Android 語音完整支援 (SpeechSynthesis Unlock)**：針對行動裝置無法自動發音的限制，在遊戲首次互動 ( `initAudio` ) 中加入了靜音語音喚醒機制，徹底解鎖手機與平板端的按鍵點讀功能。
- [x] **🔙 智能刪除鍵 (Backspace)**：重寫「清除」按鈕邏輯。不再一鍵清空所有已輸入的注音符號，改為一次刪除最後一個注音（類似倒退鍵 Backspace 的直覺操作），減少孩子們誤觸心血全消的挫敗感。

---

## 🎮 v3.5.2 已完成功能 (Phase 5: 核心漏洞修復)
- [x] **🐞 頁面切換死區修復**：修復了因動態商城面板未註冊於靜態螢幕清單中，導致「返回首頁」按鈕失效的 Bug，優化了全域的 `showScreen` 轉場邏輯。
